...
/*VARIAVEIS GLOBAIS*/
char buffer_caminho_arquivo[18]; //Caminho maximo solicitado
char output_path[18];
...
int main(void)
{
    ...
    while(flag_erro != 1) //->2<-
    {
        ...
        flag_erro = pede_arquivo(buffer_caminho_arquivo); // ->2<-
        if(flag_erro == 2)
        {
            system("cls");
...
int pede_arquivo()
{
    ...
    if (strlen(buffer_caminho_arquivo) > 1) //Se o tamanho da string for 1, ou seja, só conter o \0 - estar vazia
...
void opera_arquivo()
{
    ...
    //taxa_largura =1; //TIRAR ISSO AQUI!!!
    ...
    {
        move_cursor(0,3+q);
        for(int p = 0; p < largura+1; p++)
        {
            //ladrilho = fgetc(arquivo_input);
            ...
            if(ladrilho == '4')
                cor_vermelho += 1;          //RED
            if(ladrilho == '5')
                cor_magenta += 1;           //MAGENTA
            if(ladrilho == '6')
                cor_marrom += 1;            //BROWN
            if(ladrilho == '7')
                cor_cinza_claro += 1;       //LIGHTGRAY
            if(ladrilho == '8')
                cor_cinza_escuro += 1;      //DARKGRAY
            if(ladrilho == '9')
                cor_azul_claro += 1;        //LIGHTBLUE
            if(ladrilho == 'A')
                cor_verde_claro += 1;       //LIGHTGREEN
            if(ladrilho == 'B')
                cor_ciano_claro += 1;       //LIGHTCYAN
            if(ladrilho == 'C')
                cor_vermelho_claro += 1;    //LIGHTRED
            if(ladrilho == 'D')
                cor_magenta_claro += 1;     //LIGHTMAGENTA
            if(ladrilho == 'E')
                cor_amarelo += 1;           //YELLOW
            if(ladrilho == 'F')
                cor_branco += 1;            //WHITE

                //imprime_quadrado(taxa_largura*p+j, q+4, codigo_cor); //Imprime as coisas na tela
                //move_cursor(taxa_largura*p-3+j, q+4); //Ajusta os caracteres a tela (j é o indice de um for que incrementa taxa_largura vezes)
                move_cursor(p-1, q+4);
                //imprime_quadrado(p-1, q+4, codigo_cor);
                printf("%c", ladrilho);
        }
    }
...
void teste_passou()
{
    ...
    move_cursor(0,8);
    printf("\no caminho do arquivo eh: %s", buffer_caminho_arquivo);
    //puts(buffer_caminho_arquivo);
    move_cursor(0,10);
    system("pause");
}
