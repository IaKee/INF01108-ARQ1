...
orienta  db '>>>>>> Apos a exibicao de cada linha, digite qualquer tecla para continuar <<<<<$'
dados    ends
...
; como definir as cores de fundo e das letras no modo texto
; e como simular o uso de "janelas" no DOS

; limpa tela rolando para cima	
	mov ah,6	
	mov al,0
	mov bh,17H ; define atributo das linhas inseridas: fundo azul e letras cinza claro
	mov ch,0
	mov cl,0
	mov dh,24
	mov dl,79
...
; posiciona o cursor na primeira linha, primeira coluna da tela
	mov 	dx,0   ; dh = dl = 0
	call	pcursor
; escreve mensagem de orientacao na primeira linha da tela 
; e deixa cursor no inicio da segunda linha
	lea     dx,orienta
	mov     ah,9
	int     21h
; preenche 24 linhas com 80 caracteres de A ate X
	mov	cx,24  ; total de linhas a escrever
...
; rola a janela central da tela, com 13 linhas
	...
	mov     al,13
	mov	bh,0F4h ; preenche com  fundo cinza claro piscante (F), letras vermelhas (4)
...
; posiciona o cursor na primeira linha, primeira coluna da janela central
	mov 	dh,6 
	mov     dl,20
	call	pcursor
